// ── 导入对话框（底部抽屉，复用 ImportManager 完成实际导入）──
  // 来源：〔本地文件〕〔WebDAV〕 双卡片式（WebDAV 为远程文件管理器）
    ...
    function renderWebdav() {
      ...
      if (wd.mode === 'disconnected') {
        ...
        for (var ci = 0; ci < configs.length; ci++) {
          ...
          savedOptions += '<option value="' + escAttr(configs[ci].id) + '">' + _escHtml(optLabel) + '</option>';
        }
        wdView.innerHTML =
        ...
        for (var i = 0; i < wd.entries.length; i++) {
          ...
          if (en.isDir) {
            listHtml += '<div class="bk-webdav-dir" data-action="wd-open-dir" data-path="' + escAttr(en.remotePath) + '">' +
              ...
              '<span class="bk-webdav-name">' + _escHtml(en.name) + '</span>' +
      ...
      wdView.innerHTML =
        '<div class="bk-webdav-breadcrumb">' +
          ...
          '<span class="bk-webdav-path">' + _escHtml((nodeLabel(wd.config, wd.config && wd.config.connectedUrl) ? nodeLabel(wd.config, wd.config.connectedUrl) + ' · ' : '') + _wdDisplayPath) + '</span>' +
      ...
      var confirmHtml =
        '<div class="bk-dialog" style="width:min(340px,calc(100vw - 40px))">' +
          ...
          '<div class="bk-webdav-del-body">' +
            ...
            '<div class="bk-webdav-del-list">' + _escHtml(itemName) + '</div>' +
    ...
    function wdDeleteSelected() {
      ...
      var confirmHtml =
        '<div class="bk-dialog" style="width:min(340px,calc(100vw - 40px))">' +
          ...
          '<div class="bk-webdav-del-body">' +
            '<div class="bk-webdav-del-warn">⚠️ 删除后无法恢复</div>' +
            '<div class="bk-webdav-del-list">' + _escHtml(nameList) + '</div>' +
          ...
          '</div>' +
        '</div>';
      var delSelDlg = win.BK.openDialog({ id: 'bk-webdav-delete-confirm', html: confirmHtml });
      if (!delSelDlg) return;
      var confirmEl = document.getElementById('bk-webdav-delete-confirm');
      if (!confirmEl) return;
      var cancelBtns2 = confirmEl.querySelectorAll('[data-action="wd-del-cancel"]');
      for (var i = 0; i < cancelBtns2.length; i++) {
        cancelBtns2[i].addEventListener('click', function () { if (delSelDlg && delSelDlg.close) delSelDlg.close(); });
      }
      var confirmBtn2 = confirmEl.querySelector('[data-action="wd-del-confirm"]');
      if (confirmBtn2) {
        confirmBtn2.addEventListener('click', function () {
          if (delSelDlg && delSelDlg.close) delSelDlg.close();
          // 预置服务器删除前校验密码
          _ensureDeletePassword(function (pwd) {
